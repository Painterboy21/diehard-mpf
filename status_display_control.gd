...
func _ready() -> void:
	print("Status display scene loaded")
	...
	if USE_TEST_DATA:
		call_deferred("_show_test_preview")
	else:
...
func _make_rows() -> void:
	...
	_add_status_row("AIRPLANE", "---", x, start_y + row_gap * 0)
	_add_status_row("CENTRAL PARK", "---", x, start_y + row_gap * 1)
	_add_status_row("MULTIBALLS", "---", x, start_y + row_gap * 2)
	_add_status_row("VILLAINS", "---", x, start_y + row_gap * 3)
	_add_status_row("BULLETS", "---", x, start_y + row_gap * 4)
	_add_status_row("MYSTERY", "---", x, start_y + row_gap * 5)
	_add_status_row("SUPER JETS", "---", x, start_y + row_gap * 6)
	_add_status_row("SPINNERS", "---", x, start_y + row_gap * 7)
	_add_status_row("LOOPS", "---", x, start_y + row_gap * 8)
...
func _get_test_value(var_name: String, default_value: int = 0) -> int:
	match var_name:
		...
		"central_park_virtual_locks":
			return 1
		"nakatomi_multiball_played":
			return 1
		"airplane_multiball_played":
			return 1
		"central_park_multiball_played":
			return 0
		"hans_complete":
			return 1
		"karl_complete":
...
func _refresh_rows() -> void:
	...
	multiballs_played += _get_player_int("nakatomi_multiball_played", 0)
	multiballs_played += _get_player_int("airplane_multiball_played", 0)
	multiballs_played += _get_player_int("central_park_multiball_played", 0)
	...
	var super_jets := _get_player_int("super_jets", 0)
	var super_jets_goal := _get_player_int("super_jets_goal", 20)
	var jets_left := super_jets_goal - super_jets

	if jets_left < 0:
		jets_left = 0

	var super_spinners := _get_player_int("super_spinners", 0)
	var super_spinners_goal := _get_player_int("super_spinners_goal", 50)
	var spinners_left := super_spinners_goal - super_spinners
	...
	_set_value(0, "%d / 3" % airplane_locks)
	_set_value(1, "%d / 3" % central_park_locks)
	_set_value(2, "%d / 3" % multiballs_played)
	_set_value(3, "%d / 5" % villains_defeated)

	if bullets_left == 0:
		_set_value(4, "READY")
	else:
		_set_value(4, "%d LEFT" % bullets_left)

	if mystery_total_left == 0:
		_set_value(5, "READY")
	else:
		_set_value(5, "%d LEFT" % mystery_total_left)

	if jets_left == 0:
		_set_value(6, "READY")
	else:
		_set_value(6, "%d LEFT" % jets_left)

	if spinners_left == 0:
		_set_value(7, "READY")
	else:
		_set_value(7, "%d LEFT" % spinners_left)

	_set_value(8, str(loops_this_ball))
